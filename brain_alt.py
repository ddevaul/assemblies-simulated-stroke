...
class Area:
    ...
    def __init__(self, name, n, k, *, beta=0.05, w=0, explicit=False):
...
class Brain:
    ...
    def __init__(self, p, save_size=True, save_winners=False, seed=0):
        ...
        self.stroke_neurons = {}  # area: list of neuron indices that are dead
        ...
        for area_name in self.area_by_name:
            if self.area_by_name[area_name].explicit:
                this_stimulus_connectomes[area_name] = self._rng.binomial(
                    size, self.p, size=self.area_by_name[area_name].n
                ).astype(np.float32)
            else:
    ...
    def stroke(self, percent_neurons_destroyed=0.1, method="zero", areas=[]):
        """Give the nemo brain a stroke.
        Args:
          percent_neurons_destroyed: the percent of total neurons that will be destroyed/changed
          method: the type of destruction these neurons will undergo. if 'zero' then the weights
            will be zeroed out. TODO: if 'noise' then the weights will be randomly distrubuted
          area: the affected area which will suffer a stroke. if 'all'. then all areas will suffer
            damage. TODO: make this accept a tuple, potentially? or just accept that each area will have
            to be given a stroke separately
        """

        # for area in self.connectomes:
        #   for area2 in self.connectomes:
        #     print(self.connectomes[area][area2].shape, area)
        # gotta initialize this somehow
        if areas[0] not in self.stroke_neurons:  # not initialized
            for area in self.connectomes:
                self.stroke_neurons[area] = []

        for area in areas:
            indices = self.stroke_neurons[area]
            for i in range(self.connectomes[area][area].shape[0]):
                prob = np.random.rand()
                if prob < percent_neurons_destroyed:
                    indices.append(i)

            # indices = np.random.random_integers(0, high=)
            for idx in indices:
                for idx2 in indices:
                    self.connectomes[area][area][idx][idx2] = 0.0

            for to_area in self.connectomes[area]:
                for idx in indices:
                    for i in range(self.connectomes[area][to_area].shape[1]):
                        self.connectomes[area][to_area][idx][i] = 0.0

            for from_area in self.connectomes:
                for idx in indices:
                    for i in range(self.connectomes[from_area][area].shape[0]):
                        self.connectomes[from_area][area][i][idx] = 0.0

    # @Tyler, I think zero stroke and noisy stroke might need to be different
    # methods for clarity, but also if we do a noisy stroke I think that
    # we need to set a flag somewehre for modulation to happen, which
    # is going to be more complicated.
    def _zero_stroke(
        self, percent_neurons_destroyed: float, from_area: str, to_area: str
    ):
        """Zero out the weights in the specified area
        Args:
          area: the area to be zeroed out
        """

        # probabilities = np.random.rand(self.connectomes[from_area][to_area].shape[0], 1) # TODO: unhardcode the size?
        # self.connectomes[from_area][to_area] = np.where(probabilities <= percent_neurons_destroyed, 0.0, self.connectomes[from_area][to_area])

    def is_connectome_negative(self):
        for x in self.connectomes:
            for i in self.connectomes[x]:
                # print(i)
                for j in range(self.connectomes[x][i].shape[0]):
                    if self.connectomes[x][i].any() < 0:
                        print("fuck")
                        return
        print("nvm we're chilling")
        return False
    ...
    def add_area(self, area_name, n, k, beta):
        ...
        for other_area_name in self.area_by_name:
            ...
            new_connectomes[other_area_name] = np.empty(
                (0, other_area_size), dtype=np.float32
            )
        ...
        *,
        ...
        for stim_name, stim_connectomes in self.connectomes_by_stimulus.items():
            stim_connectomes[area_name] = self._rng.binomial(
                self.stimulus_size_by_name[stim_name], self.p, size=n
            ).astype(np.float32)
            the_area.beta_by_stimulus[stim_name] = beta
    ...
    def update_plasticities(
        self, area_update_map=EMPTY_MAPPING, stim_update_map=EMPTY_MAPPING
    ):
    ...
    def project_into(self, target_area, from_stimuli, from_areas, verbose=0):
        ...
        if target_area.fixed_assembly:
            ...
            if not target_area.explicit:
                ...
                if effective_n < target_area.k:
                    print(effective_n, target_area.n, target_area.w)
                    raise RuntimeError(
            ...
            for i in range(num_first_winners_processed):
                input_indices = rng.choice(
                    range(total_k), int(first_winner_inputs[i]), replace=False
                )
        ...
        for stim in from_stimuli:
            ...
            for i in target_area._new_winners:
                target_connectome[i] *= (
                    1 + stim_to_area_beta
                )  # TODO do we need ignore indices here?
            if verbose >= 2:
        ...
        for from_area_name in from_areas:
            ...
            for i in range(num_first_winners_processed):
                ...
                sample_indices = rng.choice(
                    from_area_winners, int(total_in), replace=False
                )
            ...
            for i in target_area._new_winners:
                for j in from_area_winners:
                    if from_area_name in self.stroke_neurons:
                        if (
                            j not in self.stroke_neurons[from_area_name]
                        ):  # TODO: figure out the wack dimensions for this thing
                            the_connectome[j, i] *= 1.0 + area_to_area_beta
                    elif target_area_name in self.stroke_neurons:
                        if i not in self.stroke_neurons[target_area_name]:
                            the_connectome[j, i] *= 1.0 + area_to_area_beta
                    else:
                        the_connectome[j, i] *= 1.0 + area_to_area_beta
            if verbose >= 2:
        ...
        for other_area_name, other_area in self.area_by_name.items():
            ...
            if other_area_name not in from_areas:
                ...
                the_other_area_connectome[:, target_area.w :] = rng.binomial(
                    ...
                    self.p,
                    size=(
                        the_other_area_connectome.shape[0],
                        target_area._new_w - target_area.w,
                    ),
                )
